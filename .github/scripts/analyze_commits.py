...
def _get_intelligent_analysis(commit_range: str) -> dict[str, Any]:
    """Load and run the intelligent commit analyzer."""
    ...
    return intelligent_analyzer.generate_intelligent_analysis(commit_range)


def _merge_intelligent_analysis(
    analysis: dict[str, Any], intelligent_analysis: dict[str, Any]
) -> None:
    """Merge intelligent-analysis output into the conventional analysis payload."""
    analysis["intelligent_analysis"] = intelligent_analysis

    analysis["stats"].update(
        {
            "intelligent_features": intelligent_analysis["content_analysis"][
                "features"
            ],
            ...
            "config_changes": len(
                intelligent_analysis["categorized_changes"]["config_changes"]
            ),
        }
    )
    ...
    analysis["categorized_commits"]["features"].extend(
        [
            ...
            for detail in intelligent_analysis["content_analysis"]["feature_details"][
                :3
            ]
        ]
    )
    ...
    analysis["categorized_commits"]["fixes"].extend(
        [
            ...
            for detail in intelligent_analysis["content_analysis"]["bug_fix_details"][
                :5
            ]
        ]
    )
...
def main():
    ...
    # Perform intelligent analysis as fallback when no conventional commits exist
    has_conventional_commits = any(analysis["stats"]["conventional_commits"].values())

    if not has_conventional_commits:
        try:
            intelligent_analysis = _get_intelligent_analysis(args.range)
